...
export class Agenda extends EventEmitter {
   ...
   */
  async database(address: string, collection?: string, options?: MongoClientOptions): Promise<Agenda> {
  ...
  private hasDatabaseConfig(config: unknown): config is (IDatabaseOptions | IMongoOptions) & IDbConfig {
   ...
   * @param skip
   */
  async jobs(query: Filter<IJobParameters> = {}, sort: Sort = {}, limit = 0, skip = 0): Promise<Job[]> {
  ...
  define(
    name: string,
    processor: ((job: Job) => Promise<void>) | ((job: Job, done: (error?: Error) => void) => void),
    options?: Partial<Pick<IJobDefinition, "lockLimit" | "lockLifetime" | "concurrency">> & {
      priority?: JobPriority;
    }
  ): void {
  ...
  async schedule<DATA = void>(when: string | Date, names: string): Promise<Job<DATA>>;
  async schedule<DATA = unknown>(when: string | Date, names: string[], data: DATA): Promise<Job<DATA>[]>;
  async schedule<DATA = unknown>(when: string | Date, name: string, data: DATA): Promise<Job<DATA>>;
  async schedule(when: string | Date, names: string | string[], data?: unknown): Promise<Job | Job[]> {

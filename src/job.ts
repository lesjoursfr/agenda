...
export class Job<DATA = unknown | void> {
  ...
  getCanceledMessage() {
    return typeof this.canceled === "object" ? this.canceled?.message || this.canceled : this.canceled;
  }
  ...
  toJson(): IJobParameters {
    ...
    for (const key of Object.keys(this.attrs)) {
      if (Object.hasOwnProperty.call(this.attrs, key)) {
        result[key as keyof IJobParameters<DATA>] =
          datefields.includes(key as TJobDatefield) && this.attrs[key as keyof IJobParameters]
            ? new Date(this.attrs[key as keyof IJobParameters] as string | number)
            : this.attrs[key as keyof IJobParameters];
      }
    }
      ...
      if (this.attrs.fork) {
        ...
        await new Promise<void>((resolve, reject) => {
          this.forkedChild = fork(
            forkHelper.path,
            [this.attrs.name, this.attrs._id!.toString(), this.agenda.definitions[this.attrs.name].filePath || ""],
            forkHelper.options
          );

...
export class JobDbRepository {
  public collection!: Collection<IJobParameters>;
  ...
  async getJobs(query: Filter<IJobParameters>, sort: Sort = {}, limit = 0, skip = 0): Promise<IJobParameters[]> {
  ...
  async lockJob(job: JobWithId): Promise<IJobParameters | undefined> {
    ...
    const resp = (await this.collection.findOneAndUpdate(criteria as Filter<IJobParameters>, update, {
  ...
  private processDbResult<DATA = unknown | void>(job: Job<DATA>, res?: IJobParameters<DATA>): Job<DATA> {
      ...
      if (props.type === "single") {
        ...
        log(
          `findOneAndUpdate(${props.name}) with type "single" ${
            result.lastErrorObject?.updatedExisting ? "updated existing entry" : "inserted new entry"
          }`
        );
        return this.processDbResult(job, result.value as IJobParameters<DATA>);
      }

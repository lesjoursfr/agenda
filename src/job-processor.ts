...
export class JobProcessor {
  private jobStatus: {
    [name: string]: { running: number; locked: number; lockLimitReached?: number } | undefined;
  } = {};
  ...
  private async findAndLockNextJob(jobName: string, definition: IJobDefinition): Promise<JobWithId | undefined> {
  ...
  private updateStatus(name: string, key: "locked" | "running" | "lockLimitReached", number: -1 | 1) {
